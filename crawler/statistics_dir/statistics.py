...
def general_statistics(df):       
    eval_col(df, "all_artists")
    eval_col(df, "norm_unique_words")

    artist_stat, artist_stat_len = calculate_counts_with_sentiment(df, "all_artists", "selected_sentiment")
    norm_words_stat_freq = calculate_counts(df, "norm_unique_words") 
    #norm_words_stat_freq_sent, _ = calculate_counts_with_sentiment(df, "norm_unique_words", "selected_sentiment")
...
def artist_statistics(df, artist_name):
    ...
    norm_words_stat = calculate_counts(df_artist, "norm_unique_words")
