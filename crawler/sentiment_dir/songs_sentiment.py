...
def extract_valid_dict(response_text, model_name, client, logger, additional_text='', max_attempts=5):
    attempt = 0
    chatgpt_messages = []
...
def find_dict(df, song_lyrics_col, tgt_col, client, logger, model_name, sentiment_prompt, additional_text):
    for ii, (idx, row) in enumerate(df.iterrows()):
        ...
        extracted_dict = extract_valid_dict(response_text, model_name, client, logger, additional_text, max_attempts=5)
        df.loc[idx, tgt_col] = str(extracted_dict)

def openai_dict_extractions(df, client, openai_model, prompt, src_col='lyrics', tgt_col='sentiment', additional_text=''):                                                                                                 
    # Establish logging
    logging.basicConfig(
        level=logging.INFO,
    ...
    find_dict(df, src_col, tgt_col, client, logger, openai_model, prompt, additional_text)
